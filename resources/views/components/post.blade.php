        ...
        <footer class="flex items-center justify-between leading-none p-2 md:p-4">
            ...
            <div>
                <div class="flex items-center text-sm">
                    ...
                    @endif
                        &nbsp;
                    <span class="flex items-center justify-between"><span class="block rounded-full w-6 h-6 bg-gray-300 text-center">{{ $post->likes->count()}} </span> {{ Str::plural('Like', $post->likes->count())}}</span>
                </div>
            </div>
            <div>
            ...
            <div>
                <div class="flex items-center text-sm">
                    <a class="no-underline hover:underline flex items-center justify-between" href="{{ route('posts.show', $post)}}"><span class="block rounded-full w-6 h-6 bg-gray-300 text-center">{{$post->comments->count()}}</span><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" />
                    </svg>{{ Str::plural('comment', $post->comments->count())}}</a>
                </div>
            </div>
        </footer>
